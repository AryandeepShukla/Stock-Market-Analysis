...
-- Updating format of Values of Dates
 update bajaj_auto
...
set `50 Day MA` = NULL limit 49;
 ...
 -- Display data of master_stock_info
...
select count(*) from infosys2 where `Signal`='BUY';
 ...
 select (select `Close price` from hero_motocorp  order by `Date` desc limit 1) - (select `Close price` from hero_motocorp  order by `Date`  limit 1) as 'Trend';
